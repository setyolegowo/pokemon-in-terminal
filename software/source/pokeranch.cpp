...
void PokeRanch::Start()
{
    ...
    while(!Exit)
    {
        ...
        switch(state)
        {
            ...
            case 8: // Menerima: 3, 4, 10, 11, 12, 13, 14, 16, 17, 18
                // -> Menampilkan area terluar
                if((command_respon >= 11) && (command_respon <= 13)) {
        ...
        switch(state)
        {
            ...
            case 5: // State 5, trainer berada di stadium
                ...
                if(command_respon > 0) { // Menerima: 3, 4, 8, 26
                    if(command_respon == 3) command_respon = 6;
                        ...
                        bet = input_manager::getNumberN(user_input,2);
                        cout << bet << " : " << user_input << " : " << Trainer.GetMoney();
                        system("pause");
                        if((bet >= 50000) && (bet <= Trainer.GetMoney())) {
                            state = 10;
                            command_respon = 255;
                        } else command_respon = 10;
                    } else command_respon = 1; // berarti menggunakan komando yang lain
                } else command_respon = 1;
                break;
            // [## SHOP ##] [## STORE ##]
                    ...
                    if(command_respon == 3) {
                        ...
                        switch(command_respon)
                        {
                            case 10:
                                ...
                                i = input_manager::getNumDirection(GetDirection);
                                if(i > 0) {
                                    outerArea.MovePlayerPosition(i,num);
            ...
            // [## BATTLE 2 ##]
            case 10: // Battle Stadium
                BattleStadium(&command_respon,bet);
                if(command_respon == 0) {
                    state = 2;
                    if(Trainer.GetDayOrNight() == 1) {
                        Trainer.AddSumOfDays(1);
...
void PokeRanch::BattleWild(unsigned char *result)
{
    ...
    if((PN < 7) && (PN <= Trainer.TotalMonster())) { // Jika monster yang availabel bertarung
        ...
        while((ActiveMonster.HP != 0) && (WildMonster.HP !=0) && (!EscapeSuccesfully) && (!SuccessUseBall))
        {
            do {
                ...
                if(command_respon > 0) {
                    if(command_respon == 3) {
                        ...
                        if(((item_use[0] >= 'A') && (item_use[0] <= 'Z')) || ((item_use[0] >= 'a') && (item_use[0] <= 'z'))) item_num_use = listItem.IsOnTable(item_use);
                        else item_num_use = 0;
                        if(((item_num_use > 0)  && (item_num_use < 8)) || ((item_num_use < 17) && (item_num_use > 11))) { 
                            if(Trainer.IsItemAda(item_num_use)) { AcceptCR = true; command_respon = 22; }
                            else { AcceptCR = false; command_respon = 1; }
                        }
                        else {
                            AcceptCR = false;
                            command_respon = 0;
                ...
                if(command_respon == 0) {
                    ...
                    command_respon = 255;
                } else if (command_respon == 1) {
                    cout << "\n\tItem tidak ditemukan dalam list pemain"; input_manager::delay(1);
                    command_respon = 255;
                }
            } while(!AcceptCR);
            if(!EscapeSuccesfully) { // ==>>Use of battle
                ...
                // *** 9 ***
                int Damage;
                while((turn <= 2) && (!SuccessUseBall)) {
                    cout << "\n\n Turn " << int(turn) << " : ";
                    // Player phase
                    if(((IsPlayerFirst && (turn == 1)) || (!IsPlayerFirst && (turn == 2))) && (!SuccessUseBall)) {
                        // *** 9a ***
    ...
    if(WildMonster.HP == 0) {
        ...
        if((Monster1.getNextEvolutionLevel() > 0) && (Monster1.getNextEvolutionLevel() <= PlayerMonster1.getCurLevel())) {
            cout << "\tMonster Anda berevolusi."; input_manager::delay(1);
            PlayerMonster1.ChangeNumberMonster(Monster1.getNextEvoNo());
            Trainer.DelMonster(PN);
            Trainer.InsertMonster(PlayerMonster1,PN);
        }
        Trainer.DelMonster(PN);
...
void PokeRanch::BattleStadium(unsigned char *result, unsigned int bet)
{
    struct MonsterActive {
        unsigned short NoMonster;
        short HPMax;
    ...
    MonsterActive WildMonster;
    MonsterActive ActiveMonster;
    unsigned short MN,PN = 1,SN = 0; // Buat nomor monster dan Party Number
    char user_input[50];
    char item_use[20];
    int item_num_use, num;
    unsigned char command_respon;
    int SkillUsePlayer;
    int SkillUseWildM;
    unsigned short MNL_strong[3] = {62,67,72};
    unsigned short MNL[3] = {63,65,61};
    unsigned char levelDefault = ((bet/25000) + 1)*4 + 10;
    if(levelDefault >= 100) levelDefault = 100;
    
    do {
        // //Mengambil no-monster liar secara acak sesuai level lantai...
        if(bet >= 500000)
            MN = (unsigned short) MNL_strong[SN];
        else
            MN = (unsigned short) MNL[SN];
        // //Mengeset Monster Liar Aktif ke variabel lokal.
        Monster MR(listMonster.getMonster(int(MN)));
        
        // *** 1 ***
        // //Melakukan konstruksi HP, Attack, Defense, Speed, dan Accuracy Monster Liar
        WildMonster.HPMax = (((MR.getHP()+ 100)*levelDefault)/50) + 10;
        WildMonster.HP = WildMonster.HPMax;
        WildMonster.Atk = (((MR.getAttack()+ 50)*levelDefault)/50) + 5;
        WildMonster.Dfn = (((MR.getDefense()+ 50)*levelDefault)/50) + 5;
        WildMonster.Spd = (((MR.getSpeed()+ 50)*levelDefault)/50) + 5;
        WildMonster.Acc = 1.0;
        WildMonsterEffect = 0;
        ...
        if((PN < 7) && (PN <= Trainer.TotalMonster())) { // Jika monster yang availabel bertarung
            ...
            while((ActiveMonster.HP != 0) && (WildMonster.HP !=0))
            {
                do {
                    // *** 4 ***
                    // Menampilkan pertarungan
                    // Wild Monster: Level, element1, percent HP, StatusEfek
                    // PlayerMonster: Level, element1, percent HP, CurrentHP/HPMax, StatusEfek, ListSkill dengan curPP/PPMax
                    interface::BattleStadiumTitle();
                    if(command_respon == 255) {
                        cout << " Level        : " << int(levelDefault) << "   " << MR.getSpesies() << endl;
                        cout << " Health Point : " << 100*WildMonster.HP/WildMonster.HPMax << "%" << endl;
                        cout << " Elemen       : " << MR.getNameElemen1() << endl; // 
                        cout << " Status Efek  : " << input_manager::getStatusEffectName(WildMonsterEffect) << endl;
                        cout.width(58);
                        ...
                        cout << "\n";
                        cout << "\n   \"LIST-PARTY\"       : Melihat daftar party.";
                        cout << "\n   \"SKILL <no-skill>\" : Menggunakan skill no <no-skill>.";
                        cout << "\n";
                    } else if(command_respon == 12) {
                        Trainer.PrintListParty();
                        Trainer.PrintInfoPartyMonster(num, &listMonster, &listSkill);
                ...
                //if(!EscapeSuccesfully) { // ==>>Use of battle
                    ...
                    while(turn <= 2) {
                            ...
                            if((int(double(UseSkill.GetAccuracy() + (((rand() % 20) + 96)/100))*ActiveMonster.Acc) > 100) && (PlayerMonster1.getStatusEffect() != 6)) {
                                ...
                                if(UseSkill.GetTarget() == 1) {
                                    ...
                                    case 0:
                                        ...
                                        break;
                                    case 1: WildMonster.Atk-=WildMonster.Atk/10; cout << "\n\t-->  Kekuatan serang monster trainer menurun.";  break;
                                    case 2: WildMonster.Dfn-=WildMonster.Dfn/10; cout << "\n\t-->  Pertahanan monster trainer menurun.";  break;
                                    case 3: WildMonster.Spd-=WildMonster.Spd/10; cout << "\n\t-->  Kecepatan monster trainer menurun.";  break;
                                    case 4: WildMonster.Acc-=WildMonster.Acc/10; cout << "\n\t-->  Akurasi monster trainer menurun.";  break;
                                    }
                                    input_manager::delay(1);
                                    // Add effect to foo
                                    if(UseSkill.GetEffect() > 0) {
                                        if((UseSkill.GetProsenEfek()*float((rand() % 20) + 95)/100) > 100) {
                                            if(WildMonsterEffect == 0){
                                                WildMonsterEffect = UseSkill.GetEffect(); 
                                                cout << "\n\t-->  Monster trainer terkena efek " << input_manager::getStatusEffectName(WildMonsterEffect); input_manager::delay(1);
                                            }
                                        }
                                    }
                                } else {
                        ...
                        if((!IsPlayerFirst && (turn == 1)) || (IsPlayerFirst && (turn == 2))) {
                            int wild_no_skill = rand() % 4;
                            while(MR.getDefaultSkills(wild_no_skill) == 0) wild_no_skill = rand() % 4;
                            cout << "\n\t--> Fase monster trainer";
                            cout << "\n\t--> Monster trainer menggunakan skill " << listSkill.GetNameSkillFromNumberSkill(MR.getDefaultSkills(wild_no_skill)); input_manager::delay(1);
                            Skill UseSkill = listSkill.GetSkill(MR.getDefaultSkills(wild_no_skill));
                            if(int(double(UseSkill.GetAccuracy() + (((rand() % 20) + 97)/100))*WildMonster.Acc) > 100) {
                                if(UseSkill.GetTarget() == 1) {
                                    switch(UseSkill.GetPowerEffect()) {
                                        case 0:
                                            if((WildMonsterEffect == 1) && ((rand()%5) > 2)) cout << "\n\t--> Monter trainer tersengat.";
                                            else if((WildMonsterEffect == 5) && ((rand()%5) > 2)) cout << "\n\t--> Monter pemain kedinginan.";
                                            else {
                                                Damage = (((((levelDefault*2/5)+2)*WildMonster.Atk*UseSkill.GetPower()/(50*ActiveMonster.Dfn))+2)*(((rand() % 20)+194)/100)*(100 - (rand() % 16))/100);
                                                if((UseSkill.GetStrWeak(1) == Monster1.getElemen0()) || (UseSkill.GetStrWeak(1) == Monster1.getElemen1())) {
                                                    Damage*=2;
                                                    cout << "\n\t--> Skill ini sangat efektif";
                                                } else if((UseSkill.GetStrWeak(2) == Monster1.getElemen0()) || (UseSkill.GetStrWeak(2) == Monster1.getElemen1())) {
                                    ...
                                    if(UseSkill.GetEffect() > 0) {
                                        if((UseSkill.GetProsenEfek()*float((rand() % 20) + 95)/100) > 100) {
                                            if(PlayerMonster1.getStatusEffect() == 0) {
                                                PlayerMonster1.ChangeEffect(UseSkill.GetEffect());
                                                cout << "\n\t--> Monster trainer terkena efek " << PlayerMonster1.getStatusEffectName();
                                            }
                                            else cout << "\n\t--> Gagal menggunakan skill efek.";
                                            input_manager::delay(1);
                                        }
                                    ...
                                    switch(UseSkill.GetPowerEffect()) {
                                    case 0:
                                        cout << "\n\t--> Health point monster trainer meningkat";
                                        if(WildMonster.HP+UseSkill.GetPower() >= WildMonster.HPMax)
                                            WildMonster.HP = WildMonster.HPMax;
                                        else
                                            WildMonster.HP+=UseSkill.GetPower();
                                        break;
                                    case 1: WildMonster.Atk+=WildMonster.Atk/10; cout << "\n\t--> Kekuatan serang monster trainer meningkat"; break;
                                    case 2: WildMonster.Dfn+=WildMonster.Dfn/10; cout << "\n\t--> Pertahanan monster trainer meningkat"; break;
                                    case 3: WildMonster.Spd+=WildMonster.Spd/10; cout << "\n\t--> Kecepatan monster trainer meningkat"; break;
                                    case 4: WildMonster.Acc+=WildMonster.Acc/10; cout << "\n\t--> Akurasi monster trainer meningkat";break;
                                    }
                                    input_manager::delay(1); 
                                }
                            } else {
                        ...
                        if((WildMonsterEffect >= 2) && (WildMonsterEffect <= 4)) {
                            ...
                            if(WildMonsterEffect == 3) {
                                ActiveMonster.HP-=ActiveMonster.HPMax/10;
                                cout << "\n\n --> Monster trainer keracunan yang sangat kuat";
                            } else if(WildMonsterEffect == 1) {
                                cout << "\n\n --> Monster trainer keracunan";
                            } else {
                                cout << "\n\n --> Monster trainer terbakar.";
                            }
                            input_manager::delay(2);
                        }
                        turn++;
    ...
    if((WildMonster.HP == 0) && (SN == 3)) {
        cout << "\n\t--> Berhasil mengalahkan monster trainer.\n"; input_manager::delay(1);
        cout << "\n\t--> Anda mendapatkan " << bet << " Zeny.\n"; input_manager::delay(1);
        Trainer.IncMoney(bet);
        Trainer.AddWin(1);
        *result = 1;
        // Get Money and Experience
    } else if(ActiveMonster.HP == 0){
        Trainer.DecMoney(bet);
        Trainer.AddLose(1);
        cout << "\n\t--> Semua monster anda kalah.\n"; input_manager::delay(1);
        // Loss money and get out to home
        *result = 0;
