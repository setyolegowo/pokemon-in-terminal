...
void City::FixUnstapableArea(){
	...
	PrintBuilding(OffsetXTele,OffsetYTele,WTeleArea,HTeleArea,8,true);
...
void City::DoTeleport(int destination){
	int temp;
	UpdatePlayerPosition(1,0); // update is needed since we want to change the lastStepped value, we move for 0 steps (equal to before)
	//convert the global appointment to local appointment
	switch(destination){
		case 1 : temp =5;
			break;
		case 2 : temp =8;
	...
	if (ValidTeleport()==4||ValidTeleport()==5||ValidTeleport()==6||ValidTeleport()==7||ValidTeleport()==8||ValidTeleport()==9||ValidTeleport()==10)
	{	
		Teleport(temp);
		LastStepped=(temp);
	}
